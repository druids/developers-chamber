...
def log_issue_time(url, username, api_key, project_key, issue_key, time_spend, comment):
    ...
    """
    log_issue_time_func(
        url, username, api_key, issue_key, time_spend, comment, project_key
    )
...
def invoke_issues_transition(url, username, api_key, jql, transition):
    ...
    click.echo(
        'Transition "{}" was invoked on issues "{}"'.format(
            transition, ", ".join(str(issue) for issue in issues)
        )
    )
