...
def start(description, workspace_id, project_id, api_key):
    ...
    click.echo(
        'Timer with description "{}" was started'.format(running_timer["description"])
    )
...
def stop(api_key):
    ...
    if stopped_timer:
        click.echo(
            'Timer with description "{}" was stopped. Duration: "{}s"'.format(
...
def print_report(workspace_id, project_id, description, from_date, to_date, api_key):
    ...
    or by date and time
    """
    report_data = get_timer_report(
        api_key, workspace_id, project_id, description, from_date.date(), to_date.date()
    )
    click.echo(
        "    {:<15}\t{}".format(
            "total time:", pretty_time_delta(report_data["total_grand"] or 0 / 1000)
        )
    )
...
def print_report_tasks(
    ...
    for timer in report_data["data"]:
        click.echo(
            "{:<15} | {}".format(pretty_time_delta(timer.dur / 1000), timer.description)
        )
