...
def start_timer(api_key, description, workspace_id=None, project_id=None):
    ...
    data = {
        "time_entry": {"created_with": client.user_agent, "description": description}
    }
    if project_id:
        data["time_entry"]["pid"] = project_id
    if workspace_id:
        data["time_entry"]["wid"] = workspace_id
    return client.decodeJSON(client.postRequest(Endpoints.START_TIME, parameters=data))[
        "data"
    ]
...
def _prepare_report_data(
    ...
    if to_date - from_date > timedelta(days=366):
        raise click.BadParameter(
            "Invalid filter parameters. You can generate reports with max 1 year timedelta"
        )
...
def get_timer_report(
    ...
    data = _prepare_report_data(
        client, workspace_id, project_id, description, from_date, to_date
    )
    return client.getDetailedReport(data)


def get_full_timer_report(
    api_key,
    workspace_id=None,
    project_id=None,
    description=None,
    ...
    pages_number = (
        ...
        if report_data.get("per_page", 0) != 0
        else 0
    )
